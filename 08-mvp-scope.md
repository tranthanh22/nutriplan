...
| MVP-02 | Tính BMR/TDEE/Macro | Một công thức được xác nhận; hệ số vận động; ba mục tiêu giảm/duy trì/tăng; lưu phiên bản công thức | Tạo dữ liệu cá nhân hóa đáng tin cậy cho thực đơn và AI |
| MVP-02A | AI Health Insights | AI giải thích dữ liệu đã tính bằng output có cấu trúc; free xem tóm tắt, subscriber xem đầy đủ; không chẩn đoán | Biến số liệu khó hiểu thành giá trị dễ cảm nhận và hỗ trợ chuyển đổi subscription |
...
| MVP-04 | NutriPlan Subscription | Ba thời hạn 7 ngày, 1 tháng và 3 tháng; thanh toán test hoặc xác nhận thanh toán thủ công; trạng thái active/expired/cancel-at-period-end | Kiểm chứng willingness-to-pay và thời hạn người dùng ưu tiên |
...
|---|---|---|
| Tự động gia hạn, đổi gói giữa chu kỳ và hoàn tiền tự động | MVP cho chọn ba thời hạn nhưng mỗi giao dịch trả trước một lần, chưa xử lý billing phức tạp | Sau pilot |
...
| AIHealthInsight | User, NutritionProfile, input/output JSON, model, prompt/formula version, status, safety status |
...
| Backend | Các module Nutrition, AI Insights, Meal Plan, Subscription, Kitchen Marketplace, Order, Meal Log |
| Cơ sở dữ liệu | Cơ sở dữ liệu quan hệ |
| Gợi ý thực đơn | Rule-based trên kho món được chuẩn hóa; chưa cần mô hình AI |
| AI Health Insights | NestJS gọi provider phía server, Structured Outputs/JSON Schema, cache theo phiên bản hồ sơ và fallback khi lỗi |
...
| Sprint 1 | Cá nhân hóa và preview | Nutrition Profile, BMR/TDEE/Macro, AI Insight có cấu trúc, lọc allergen, preview thực đơn |
...
| MVP quá lớn | Cao | Chỉ một bếp, ba thời hạn trả trước dùng chung quyền lợi, gói bếp 5 ngày; ảnh là Should Have |
